...
pub extern "C" fn verify(
    difficulty: c_uint,
    input:  *const c_uchar,  input_size: size_t,
    output: *const c_uchar, output_size: size_t,
    _proof:  *const c_uchar,  _proof_size: size_t,
    size_in_bits: size_t
) -> u8 /*bool*/ {
    ...
    let input_slice = unsafe { slice::from_raw_parts(input as *const u8, input_size as usize) };
    let output_slice = unsafe { slice::from_raw_parts(output as *const u8, output_size as usize) };

    let wesolowski_vdf = WesolowskiVDFParams(size_in_bits as u16).new();
    
    
    let success = wesolowski_vdf.verify(input_slice, difficulty as u64, output_slice).is_ok();
